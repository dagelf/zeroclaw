//! CLI and channel entrypoints for the agent loop.
//!
//! - `run()` — full CLI/daemon initialization + interactive REPL or single-shot execution.
//! - `process_message()` — simplified channel path (Telegram, Discord, etc.).

use super::{
    AUTOSAVE_MIN_MESSAGE_CHARS, DraftEvent, agent_turn, autosave_memory_key, build_context,
    build_hardware_context, build_tool_instructions, clear_model_switch_request,
    compute_excluded_mcp_tools, get_model_switch_state, is_model_switch_requested,
    is_tool_loop_cancelled, load_interactive_session_history, run_tool_call_loop,
