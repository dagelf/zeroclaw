...
//!
//! Supports multiple formats: OpenAI JSON, XML tags, markdown fences,
//! GLM line-based, Perl-style, and MiniMax invoke format.
...
mod tests {
    ...
    // ── canonicalize_json_for_tool_signature tests ───────────────────────

    #[test]
    fn canonicalize_sorts_keys_alphabetically() {
        let input: serde_json::Value = serde_json::json!({"z": 1, "a": 2, "m": 3});
        let result = canonicalize_json_for_tool_signature(&input);
    ...
    fn canonicalize_handles_nested_objects() {
        let input: serde_json::Value = serde_json::json!({"b": {"z": 1, "a": 2}, "a": 1});
        let result = canonicalize_json_for_tool_signature(&input);
        let keys: Vec<&String> = result.as_object().unwrap().keys().collect();
        assert_eq!(keys, vec!["a", "b"]);
        let inner_keys: Vec<&String> = result["b"].as_object().unwrap().keys().collect();
        assert_eq!(inner_keys, vec!["a", "z"]);
    }

    #[test]
    fn canonicalize_handles_arrays() {
        let input: serde_json::Value = serde_json::json!([{"b": 1, "a": 2}]);
        let result = canonicalize_json_for_tool_signature(&input);
        let inner_keys: Vec<&String> = result[0].as_object().unwrap().keys().collect();
        assert_eq!(inner_keys, vec!["a", "b"]);
    }

    #[test]
    fn canonicalize_preserves_primitives() {
        assert_eq!(
            canonicalize_json_for_tool_signature(&serde_json::json!("hello")),
            serde_json::json!("hello")
        );
        assert_eq!(
            canonicalize_json_for_tool_signature(&serde_json::json!(42)),
            serde_json::json!(42)
        );
        assert_eq!(
            canonicalize_json_for_tool_signature(&serde_json::json!(null)),
    ...
    #[test]
    fn find_json_end_simple_object() {
        assert_eq!(find_json_end(r#"{"a": 1}"#), Some(8));
    }

    #[test]
    fn find_json_end_nested_objects() {
        assert_eq!(find_json_end(r#"{"a": {"b": 1}}"#), Some(15));
    }

    #[test]
    fn find_json_end_with_escaped_braces_in_string() {
        assert_eq!(find_json_end(r#"{"a": "val\"}ue"}"#), Some(17));
    }

    #[test]
    fn find_json_end_no_opening_brace() {
        assert_eq!(find_json_end("no json here"), None);
    }

    #[test]
    fn find_json_end_unclosed_brace() {
        assert_eq!(find_json_end(r#"{"a": 1"#), None);
    }

    #[test]
    fn find_json_end_with_leading_whitespace() {
        assert_eq!(find_json_end("  {\"a\": 1}"), Some(10));
    }

    // ── strip_leading_close_tags tests ──────────────────────────────────

    #[test]
    fn strip_leading_close_tags_removes_xml_close() {
        assert_eq!(
            strip_leading_close_tags("</tool_call>{\"a\": 1}"),
            "{\"a\": 1}"
        );
    }

    #[test]
    fn strip_leading_close_tags_removes_multiple() {
        assert_eq!(strip_leading_close_tags("</foo></bar>content"), "content");
    }

    #[test]
    fn strip_leading_close_tags_no_tags() {
        assert_eq!(strip_leading_close_tags("no tags here"), "no tags here");
    }

    #[test]
    fn strip_leading_close_tags_whitespace_before_tag() {
        assert_eq!(strip_leading_close_tags("  </tag>content"), "content");
    }

    // ── extract_first_json_value_with_end tests ─────────────────────────

    #[test]
    fn extract_first_json_valid_object() {
        let (val, end) = extract_first_json_value_with_end(r#"{"name": "shell"}"#).unwrap();
        assert_eq!(val["name"], "shell");
        assert_eq!(end, 17);
    }

    #[test]
    fn extract_first_json_with_trailing_text() {
        let (val, end) = extract_first_json_value_with_end(r#"{"a": 1} trailing text"#).unwrap();
        assert_eq!(val["a"], 1);
        assert_eq!(end, 8);
    }

    #[test]
    fn extract_first_json_no_json() {
        assert!(extract_first_json_value_with_end("no json here").is_none());
    }

    #[test]
    fn extract_first_json_partial_json() {
        assert!(extract_first_json_value_with_end(r#"{"unclosed"#).is_none());
    }

    // ── parse_xml_attribute_tool_calls tests ─────────────────────────────

    #[test]
    fn parse_xml_attribute_single_invoke() {
        let input = r#"<invoke name="shell"><parameter name="command">ls</parameter></invoke>"#;
        let calls = parse_xml_attribute_tool_calls(input);
    ...
    #[test]
    fn parse_xml_attribute_multiple_params() {
        let input = r#"<invoke name="file_write"><parameter name="path">/tmp/test</parameter><parameter name="content">hello</parameter></invoke>"#;
        let calls = parse_xml_attribute_tool_calls(input);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].arguments["path"], "/tmp/test");
        assert_eq!(calls[0].arguments["content"], "hello");
    }

    #[test]
    fn parse_xml_attribute_empty_name() {
        let input = r#"<invoke name=""><parameter name="x">y</parameter></invoke>"#;
        let calls = parse_xml_attribute_tool_calls(input);
        assert!(calls.is_empty());
    }

    #[test]
    fn parse_xml_attribute_no_match() {
        let calls = parse_xml_attribute_tool_calls("plain text with no invoke tags");
        assert!(calls.is_empty());
    }

    // ── parse_function_call_tool_calls tests ─────────────────────────────

    #[test]
    fn parse_function_call_single() {
        let input = "<FunctionCall> file_read <code>path>/etc/hosts</code> </FunctionCall>";
        let calls = parse_function_call_tool_calls(input);
        assert_eq!(calls.len(), 1);
    ...
    #[test]
    fn parse_function_call_no_match() {
        let calls = parse_function_call_tool_calls("no function calls here");
        assert!(calls.is_empty());
    }

    #[test]
    fn parse_function_call_maps_aliases() {
        let input = "<FunctionCall> bash <code>command>ls -la</code> </FunctionCall>";
        let calls = parse_function_call_tool_calls(input);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].name, "shell");
    }

    // ── is_xml_meta_tag tests ───────────────────────────────────────────

    #[test]
    fn is_xml_meta_tag_recognizes_known_tags() {
        assert!(is_xml_meta_tag("tool_call"));
        assert!(is_xml_meta_tag("toolcall"));
        assert!(is_xml_meta_tag("tool-call"));
        assert!(is_xml_meta_tag("invoke"));
        assert!(is_xml_meta_tag("thinking"));
    }

    #[test]
    fn is_xml_meta_tag_case_insensitive() {
        assert!(is_xml_meta_tag("TOOL_CALL"));
        assert!(is_xml_meta_tag("Invoke"));
        assert!(is_xml_meta_tag("THINKING"));
    }
    ...
    #[test]
    fn is_xml_meta_tag_rejects_unknown() {
        assert!(!is_xml_meta_tag("div"));
        assert!(!is_xml_meta_tag("function"));
        assert!(!is_xml_meta_tag("shell"));
    }

    // ── parse_minimax_invoke_calls tests ─────────────────────────────────

    #[test]
    fn parse_minimax_invoke_single_call() {
        let input = r#"<invoke name="shell"><parameter name="command">ls</parameter></invoke>"#;
        let result = parse_minimax_invoke_calls(input);
    ...
    #[test]
    fn parse_minimax_invoke_with_surrounding_text() {
        let input = r#"Here is the result: <invoke name="shell"><parameter name="command">pwd</parameter></invoke> Done."#;
        let result = parse_minimax_invoke_calls(input);
        assert!(result.is_some());
        let (text, calls) = result.unwrap();
    ...
    #[test]
    fn parse_minimax_invoke_no_match() {
        let result = parse_minimax_invoke_calls("plain text with no invoke");
    ...
    #[test]
    fn build_curl_command_valid_url() {
        let cmd = build_curl_command("https://example.com/api").unwrap();
        assert!(cmd.contains("curl"));
        assert!(cmd.contains("https://example.com/api"));
    }

    #[test]
    fn build_curl_command_non_http_returns_none() {
        assert!(build_curl_command("ftp://example.com").is_none());
        assert!(build_curl_command("not-a-url").is_none());
    }

    // ── parse_tool_call_id tests ────────────────────────────────────────

    #[test]
    fn parse_tool_call_id_from_root() {
        let root = serde_json::json!({"id": "call_123", "name": "shell"});
        assert_eq!(
            parse_tool_call_id(&root, None),
            Some("call_123".to_string())
        );
    }

    #[test]
    fn parse_tool_call_id_from_function() {
        let root = serde_json::json!({"name": "shell"});
        let func = serde_json::json!({"id": "func_456"});
        assert_eq!(
            parse_tool_call_id(&root, Some(&func)),
            Some("func_456".to_string())
        );
    }

    #[test]
    fn parse_tool_call_id_empty_returns_none() {
        let root = serde_json::json!({"id": "", "name": "shell"});
        assert_eq!(parse_tool_call_id(&root, None), None);
    }

    #[test]
    fn parse_tool_call_id_missing_returns_none() {
        let root = serde_json::json!({"name": "shell"});
        assert_eq!(parse_tool_call_id(&root, None), None);
    }
}
