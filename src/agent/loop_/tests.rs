...
async fn cost_tracking_records_usage_when_scoped() {
    use super::{TOOL_LOOP_COST_TRACKING_CONTEXT, ToolLoopCostTrackingContext, run_tool_call_loop};
...
#[test]
fn compute_excluded_mcp_tools_empty_groups_returns_empty() {
    let result = super::compute_excluded_mcp_tools(&[], &[], "any message");
    assert!(result.is_empty());
}

// ── scope_thread_id tests ───────────────────────────────────────────

#[tokio::test]
async fn scope_thread_id_sets_and_unsets_task_local() {
    use super::TOOL_LOOP_THREAD_ID;

    // Outside scope, thread_id should not be set.
    let outside = TOOL_LOOP_THREAD_ID.try_with(|id| id.clone());
    assert!(outside.is_err());

    // Inside scope, thread_id should be accessible.
    let result = super::scope_thread_id(Some("test-thread".to_string()), async {
        TOOL_LOOP_THREAD_ID
            .try_with(|id| id.clone())
            .unwrap_or(None)
    })
    .await;
    assert_eq!(result, Some("test-thread".to_string()));
}

#[tokio::test]
async fn scope_thread_id_none_sets_none() {
    use super::TOOL_LOOP_THREAD_ID;

    let result = super::scope_thread_id(None, async {
        TOOL_LOOP_THREAD_ID
            .try_with(|id| id.clone())
            .unwrap_or(Some("fallback".to_string()))
    })
    .await;
